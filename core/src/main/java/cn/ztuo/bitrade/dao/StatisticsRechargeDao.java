...
import cn.ztuo.bitrade.entity.statistics.StatisticsRecharge;
 ...
 * @author Paradise
 */
public interface StatisticsRechargeDao extends BaseDao<StatisticsRecharge> {
    ...
    @Query(value = "SELECT sum(people_count) as people_count,sum(recharge_count) recharge_count,sum(recharge_amount) recharge_amount " +
            "FROM statistics_recharge s WHERE if(?3 != '' and ?3 is not null ,s.currency = ?3,1) and " +
